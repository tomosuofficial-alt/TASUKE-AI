...
└────────────────────────
Veo使えない日 → CapCutで「Ken Burns効果」（3秒で1.2倍ズーム）

【STEP 4: テキストを入れる】
ツール: CapCut
やること: フック文を冒頭1秒目に配置（白文字＋影つき、画面上部1/3）

【STEP 5: 仕上げ・書き出し】
ツール: CapCut
やること: BGMを追加 → 書き出し（1080x1920, MP4）`,
...
└────────────────────────
Veo使えない日 → CapCutで「ズームイン」効果（2秒→3秒かけてゆっくり寄る）

【STEP 4: テキストを入れる】
ツール: CapCut
やること: フック文を画面下部1/4に控えめに配置（白文字＋影）
...
【STEP 4: スライド動画にする】
ツール: CapCut
やること: STEP2（スイーツ）→ STEP3（ドリンク）→ 店舗情報画面 の3枚をスライド切替
各カット2〜3秒。フック文は冒頭1秒目に配置
...
【STEP 2: 各写真を加工する】
ツール: Google AI Studio → ImageFX
やること: 各写真を1枚ずつアップロード → 下のプロンプトをコピペ入力
┌────────────────────────
...
【STEP 3: テキストを追加する】
ツール: Canva
やること: 9:16テンプレートにSTEP2の写真を配置 → 価格・セット内容をテキスト追加

【STEP 4: スライドショー動画にする】
ツール: CapCut
やること: 各料理を2〜3秒ずつ表示するスライドショーにする
フック文は冒頭1秒目に配置
...
└────────────────────────
Veo使えない日 → CapCutで「パン」効果（左→右にゆっくりスライド）

【STEP 4: テキストを入れる】
ツール: CapCut
やること: フック文を冒頭1秒目に配置（白文字＋影）
...
└────────────────────────
Veo使えない日 → CapCutで「ズームイン」効果（パティ寄り→引きの完成形）
...
【STEP 3: テキストを追加する】
ツール: Canva
やること: 各画像にメニュー名＋価格をテキスト追加

【STEP 4: スライドショー動画にする】
ツール: CapCut
やること: 3品を順番に2〜3秒ずつ表示するスライドショーにする
フック文は冒頭1秒目に配置
...
【STEP 2: 内装画像を加工する】
ツール: Google AI Studio → ImageFX
やること: 内装写真をアップロード → 下のプロンプトをコピペ入力
┌────────────────────────
...
【STEP 3: ドリンク画像を加工する】
ツール: Google AI Studio → ImageFX
やること: ドリンク写真をアップロード → 下のプロンプトをコピペ入力
┌────────────────────────
...
└────────────────────────
Veo使えない日 → CapCutで「パン」効果（内装→ドリンクにフォーカス）
...
async function main() {
  ...
  for (const [clientName, config] of Object.entries(CLIENT_CONFIGS)) {
    console.log(`📱 ${clientName}（${config.postsPerMonth}本/月）`);

    const dates = calculatePostDates(year, month, config.baseDays);

    for (let i = 0; i < dates.length; i++) {
      const success = await createPostEntry(config, clientName, i, dates[i], month);
      if (success) {
        totalCreated++;
      } else {
        totalFailed++;
