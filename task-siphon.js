 ...
 *   TASK_SIPHON_MODEL 既定 haiku（alias）。sonnet / opus / claude-haiku-4-5-20251001 等も可
...
const MODEL = process.env.TASK_SIPHON_MODEL || 'haiku';
...
const SIPHON_SCHEMA = {
  ...
  required: ['overlooked_or_implicit', 'blockers_or_unclear', 'summary_for_user'],
  properties: {
    overlooked_or_implicit: {
      type: 'array',
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['title', 'kind', 'urgency', 'who_should_act', 'deadline_note', 'client_hint', 'evidence'],
        properties: {
          title: { type: 'string' },
          kind: { type: 'string', enum: ['task', 'follow_up', 'decision_needed'] },
          urgency: { type: 'string', enum: ['high', 'medium', 'low', 'unknown'] },
          who_should_act: { type: 'string', enum: ['自分', '相手', '双方', 'unknown'] },
...
async function main() {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      const { result, text: raw } = await runClaude({
        prompt: userPrompt,
        systemPrompt: sys,
        schema: SIPHON_SCHEMA,
        model: MODEL,
      });
      data = result;
      ...
      throw new Error(`想定外の応答: ${String(raw).slice(0, 400)}`);
    } catch (e) {
      if (attempt === 0) {
