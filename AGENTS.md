...
Codex 用の定義は `.Codex/agents/*.md` に別途存在（共存）。

### チーム制（依頼の振り分けルール）

幹部ごとの職能分担に加えて、依頼の振り分けは次の3チームで整理する。チームは新しい人格や常駐エージェントではなく、主担当を決めるための分類ルールとして扱う。

| チーム | 役割 | 所属幹部 |
|------|------|----------|
| Growth | 集客・SNS・競合・提案・クリエイティブ | ミセル、ヒビキ、サグル、エガク |
| Ops | Notion・Slack・日次運用・請求・売上管理・業務設計 | マワリ、カネル |
| Build | 自動化・API連携・スクリプト・GitHub Actions・技術実装 | ツクル、マワリ |

#### チーム振り分けルール

- **器を作る仕事**は Ops が主導する（Notion DB、Slack通知、日次ブリーフ、案件管理、運用フローなど）。
- **中身を伸ばす仕事**は Growth が主導する（投稿企画、Instagram運用、競合調査、集客施策、提案内容など）。
- **仕組み化する仕事**は Build が主導する（API連携、スクリプト、自動化、GitHub Actions、データ同期など）。
- 複数チームにまたがる依頼は、主担当チームを1つ決め、他チームは協力として参加する。

例:

| 依頼 | 主担当 | 協力 |
|------|--------|------|
| NotionのコンテンツDB作成 | Ops | Growth要件、Build自動化 |
| Instagram投稿カレンダー作成 | Growth | Ops保存、Build自動生成 |
| freee売上KPI自動取得 | Ops | Build実装、Growth参照 |
